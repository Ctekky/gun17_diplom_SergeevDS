...
namespace Metroidvania.Common.Rope
{
    public class RopeLinks : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            GetComponent<HingeJoint2D>().anchor = new Vector2(0,
                -transform.parent.GetComponentInChildren<SpriteRenderer>().sprite.bounds.extents.y);
            if (transform.GetSiblingIndex() == 0)
            {
                GetComponents<HingeJoint2D>()[1].anchor =
                    new Vector2(0, GetComponent<SpriteRenderer>().sprite.bounds.extents.y);
            }
            else
            {
                GetComponent<CapsuleCollider2D>().size =
                    transform.parent.GetComponentInChildren<CapsuleCollider2D>().size;
                GetComponent<CapsuleCollider2D>().offset =
                    transform.parent.GetComponentInChildren<CapsuleCollider2D>().offset;
            }
