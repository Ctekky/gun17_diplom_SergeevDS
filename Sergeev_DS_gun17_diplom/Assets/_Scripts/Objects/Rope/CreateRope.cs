...
namespace Metroidvania.Common.Rope
{
    public class CreateRope : MonoBehaviour
    {
        ...
        [Header("Rope Settings")] [Range(2, 50, order = 1)] [Tooltip("Количество сегментов")] [Delayed]
        public int ropeSegments;

        public bool hideLastRope;
        public bool autoAssignRopeAttachPositions;
        [Min(2)] public int numberOfRopeAttachPositions;
        ...
        [SerializeField] public GameObject LastRope { get; private set; }
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            Gizmos.DrawWireCube(firstSegment.transform.position,
                new Vector2(0, firstSegment.GetComponent<SpriteRenderer>().bounds.size.y - ropeAttachPositionOffset));
        }
        ...
        private void BuildRope()
        {
            for (var i = 0; i < ropeSegments - 1; i++)
            {
                var additionalOffset = Mathf.Abs(_links.GetChild(i).GetComponent<RopeLinks>().transform.position.y -
                                                 _links.GetChild(i).GetComponent<RopeLinks>().ConnectionPoint.position
                                                     .y) - 0.01f;
                _spawnLocation = (Vector2)_links.GetChild(i).GetComponent<RopeLinks>().ConnectionPoint.position -
                                 new Vector2(0, additionalOffset);
                var checkGround = Physics2D.Raycast(_spawnLocation, Vector2.down,
                    firstSegment.GetComponent<SpriteRenderer>().bounds.size.y * 2f, groundLayer);
